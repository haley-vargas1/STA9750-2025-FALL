...
download_nyc_council_boundaries <- function(){
  ...
  if(!dir.exists(unzip_dir)){
    message("🗂️ Unzipping shapefile data...")
  ...
  if(length(shp_file) == 0){
    stop("❌ No shapefile found after unzipping. Check folder structure inside: ", unzip_dir)
  }
  ...
  # Transform to WGS84
  message("🌍 Transforming to WGS84 CRS...")
  nyc_council_wgs84 <- sf::st_transform(nyc_council, crs = "WGS84")
  
  message("✅ NYC Council District Boundaries ready.")
...
get_nyc_trees <- function(n_max = Inf) {
  ...
  message("🌳 Starting NYC Tree Points download...")
  
  while(!finished){
    
    # Stop early if we reached n_max
    if(total_downloaded >= n_max){
      message("🛑 Reached n_max limit, stopping early.")
      break
    }
    ...
    if(!file.exists(batch_file)){
      message(paste0("📦 Downloading batch at offset ", start_offset, "..."))
      ...
      message(paste0("✅ Batch at offset ", start_offset, " already exists."))
    }
    ...
    if(nrow(trees_chunk) == 0){
      message("🛑 No more data returned from API.")
      finished <- TRUE
    } else {
      ...
      message(paste0("✅ Downloaded ", nrow(trees_chunk),
  ...
  message("🔗 Combining downloaded batches...")
  all_trees <- dplyr::bind_rows(tree_batches)
  
  message(paste0("🌲 Final dataset contains ", nrow(all_trees), " trees."))
...
# 1. 🌳 Proposal: Dead Tree Replacement Program — NYC District 51 🌳
